...
function Topline_DisplayProductDetailsLoginLink($intClientRID,$intServiceRID,$blnReturnIfLoginURLCanBeCreated = false)
{
	...
	//-----------------------------
	$servicedata = mysql_fetch_array(select_query("tblhosting","packageid,server",array("id"=>$intServiceRID,"userid"=>$intClientRID)));
	...
	if($intPackageRID > 0)
	{
		...
		if($intYolaProductBundleRID > 0)
		{
			...
			if($blnReturnIfLoginURLCanBeCreated == true)
				...
				return Topline_DisplayErrorMessage("<script>alert('There was a issue trying to find your sitebuilder plan, please contact us for help. Error 1');window.close();</script>");
	}else
		if($blnReturnIfLoginURLCanBeCreated == true)
			return false;
		else
			return Topline_DisplayErrorMessage("<script>alert('There was a issue trying to find your hosting plan, please contact us for help. Error 2');window.close();</script>");
}
//-----------------------------------------------
...
class ToplineAPI {
	...
	*/
	...
	private function CallService($intCallType,$aryFields)
	{
		...
		//logModuleCall("sitebuilder","ToplineAPI:CallService","Calling Service ID: $intCallType, URL: $url, with results:". curl_error($this->curl),"");
	...
	if(isset($commits[0]["commit"]["url"]))
	{
		...
		if(isset($commitsData["commit"]["message"]))
		{
			if(strpos($commitsData["commit"]["message"],"\n") !== false)
			{
				...
				$strCurrentVersion = trim($aryMessageData[0]);
			}else{
				$strCurrentVersion = $commitsData["commit"]["message"];
			}
			$strCurrentVersion = strtolower($strCurrentVersion);
			if(strpos($strCurrentVersion,".") !== false) {
				...
				$strCurrentVersion = str_replace("v","",$strCurrentVersion);
				$strCurrentVersion = str_replace(":","",$strCurrentVersion);
			}else{
				$strCurrentVersion = "";
			}
